...
mongoose.connect("mongodb://127.0.0.1:27017/combinedApp", {
...
app.post("/login", async (req, res) => {
  try {
    ...
    if(inputPassword === storedPassword) {
      res.redirect("/views/customerlink"); // Redirect to home page after successful login
    } else {
...
// Existing code from the first Express app
...
// Existing QR code generation route
...
app.get("/paymentDetails", async (req, res) => {
  try {
    ...
    res.render("paymentDetails", { latestPaymentDetails });
  } catch (error) {
